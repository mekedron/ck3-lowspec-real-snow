...
патч читает опцию этого дополнения, но не действует по ней: маска снега AGOT выключает
материал снега везде на той карте, так что патч рисует вместо него снег самого AGOT.
...
краска поверх земли. С опцией Sharp Terrain вызывает `ApplySnowMaterialTerrainCheap()`:
ванильный high-spec `ApplySnowMaterialTerrain()`, без синусного шума и работы с производными его пяти `SampleNoTile` (две выборки масок
оставляют по два усреднённых чтения, три выборки материала становятся одним чтением
каждая) и без второго чтения карты высот для горного члена, потому что высота у пиксельного
шейдера уже есть. Материал снега так же вмешивается в per-pixel высоту, нормаль и
материал деталей: снег получает свой рельеф и блеск, плюс слой инея по краям. Около 9
чтений текстур на пиксель снега вместо примерно 14 плюс шумовая математика; текстура
снега повторяется со своим тайлингом, а не перемешивается, на почти однородной белой
текстуре это не видно. `TERRAINOPT_SNOW_MATERIAL_VANILLA` в файле опций возвращает
ванильную функцию, для сравнения.

## Цена

Ванильный материал стоил около **4 мс на кадр** там, где снег возможен; замерено на
RTX 3050 Ti Laptop (4 ГБ) при 5120x1440 в январе, а на Mac заметно больше. Дешёвая
версия делает меньше чтений текстур и не считает шум. Там, где маска
говорит «снега не бывает», шейдер выходит после одной выборки.
...
Собран под 1.19.0.6 (Scribe) и Sharp Terrain 1.2 (дешёвая функция снега живёт там,
с 1.1 рисуется ванильный материал). У файла нет ванильного аналога,
