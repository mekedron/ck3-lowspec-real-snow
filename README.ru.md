...
ванильный high-spec `ApplySnowMaterialTerrain()`, в котором каждый `SampleNoTile` (два
чтения текстуры плюс синусный шум, пять штук на пиксель) заменён одним обычным чтением,
и без второго чтения карты высот для горного члена, потому что высота у пиксельного
шейдера уже есть. Материал снега так же вмешивается в per-pixel высоту, нормаль и
материал деталей: снег получает свой рельеф и блеск, плюс слой инея по краям. Около 7
...
версия делает примерно вдвое меньше чтений текстур и не считает шум. Там, где маска
