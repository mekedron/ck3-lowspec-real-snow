...
TARGETS = [
    ...
    ('pdxterrain.shader', 'PdxTerrainLowSpec', True, HOME + '/Projects/ck3-lowspec-terrain-fix/gfx/FX/pdxterrain.shader'),
...
def switch_block():
    p = os.path.join(MOD, 'gfx/FX/fastadv.fxh')
    if not os.path.exists(p):
        return ''
    t = read(p)
    return t[t.index('[[') + 2: t.index(']]')]
